...
class JournalEntry extends HTMLElement {
  ...
  set entry(entry) {
    ...
    // CODE GOES HERE
    const entryShownDOM = this.shadowRoot;
    const entryArticle = entryShownDOM.querySelector('article');
    const paras = entryShownDOM.querySelectorAll('p');
    entryShownDOM.querySelector('h2').textContent = entry.title;
    paras[0].textContent = entry.date;
    paras[1].textContent = entry.content;
